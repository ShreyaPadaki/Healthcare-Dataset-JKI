...
# Stop the previous localtunnel process (if running)
!pkill localtunnel
...
# If you don't have one, you can sign up at ngrok.com and get one.
# Add it to Colab secrets with the name 'NGROK_AUTH_TOKEN'
try:
    NGROK_AUTH_TOKEN = userdata.get('NGROK_AUTH_TOKEN')
except Exception as e:
    print(f"Error retrieving NGROK_AUTH_TOKEN secret: {e}")
...
if NGROK_AUTH_TOKEN:
    ...
    print("NGROK_AUTH_TOKEN secret not found. Please add it to Colab secrets to authenticate ngrok.")
    # Raise an error or exit if the token is mandatory
    # For now, we'll let the ngrok.connect potentially fail with an auth error if token is missing
    # raise ValueError("NGROK_AUTH_TOKEN not set. Please add it to Colab secrets.") # Uncomment to make it mandatory

# Terminate any existing ngrok tunnels
ngrok.kill()

# Start a new ngrok tunnel for Streamlit's default port 8501
print("Starting ngrok tunnel...")
# Use connect with the port. This will fail if auth is required and not set.
try:
    public_url = ngrok.connect(8501).public_url
    print(f"ngrok tunnel established at: {public_url}")
except Exception as e:
    print(f"Error establishing ngrok tunnel: {e}")
    print("Please ensure your NGROK_AUTH_TOKEN is correctly set in Colab secrets and is valid.")
    public_url = None # Set public_url to None if connection fails


# Run the Streamlit app in a separate cell
# This command will block until the Streamlit app is stopped.
# The ngrok tunnel will remain active as long as the ngrok connect cell is running.
# The streamlit run command should be in a separate cell to keep the ngrok tunnel alive

# The original code had the streamlit run command here, moving it to a separate cell W1rhHfb-dswT
# !streamlit run app.py --server.port 8501 --server.enableCORS false --server.enableXsrfProtection false

# Commented out IPython magic to ensure Python compatibility.
# This cell runs the streamlit app after the ngrok tunnel is established
# Change directory to /content where app.py is saved
# %cd /content
# Add commands to check the current directory and list files
!pwd
!ls
